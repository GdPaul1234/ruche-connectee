...
import { getBatteryResponse, TemperatureHumidityResponse } from "../services/hive.service"
...
export async function loader({ request, params }: LoaderArgs) {
  ...
  if (params.hiveId) return getBatteryResponse(+params.hiveId, +start, +stop)
}

export default function HiveBatteryPage() {
  const [searchParams] = useSearchParams()
  const temperatures = useLoaderData() as TemperatureHumidityResponse
  const batteryData = apiSensorResponseToChartData(temperatures)
  ...
  return <article className="w-full">
    ...
    <ChartLineComponent data={batteryData} />
  </article>
}
